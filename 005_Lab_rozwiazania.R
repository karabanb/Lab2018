...
# caret na dzien 13-11-18 nie chce sie zainstalowac, robimy obejscie pakietem mlr
...
# classif_task <- makeClassifTask(data = as.data.frame(dataset_classif), target = "IfPayment6M", positive = "0")
# rdesc <- makeResampleDesc(method = "Holdout", stratify = TRUE, split = 0.7)
# ix <- makeResampleInstance(desc = rdesc, task = classif_task)

# train_ix <- unlist(ix$train.inds)
